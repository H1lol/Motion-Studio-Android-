...
class MainActivity : Activity(), TimelineHost {
    ...
    // -------------------------------------------------------------------------
    // COMPACT  = phones in landscape (short-side < 600dp)
    // MEDIUM   = small/7" tablets and unfolded compact foldables
    // EXPANDED = 10"+ tablets, large foldables
    ...
    private fun leftPanelWidthDp(): Int = when (screenClass()) {
        ScreenClass.COMPACT -> 150
        ScreenClass.MEDIUM -> 190
        ScreenClass.EXPANDED -> 240
    }

    private fun rightPanelWidthDp(): Int = when (screenClass()) {
        ScreenClass.COMPACT -> 170
        ScreenClass.MEDIUM -> 220
        ScreenClass.EXPANDED -> 280
    }

    private fun timelinePanelHeightDp(): Int = when (screenClass()) {
        ScreenClass.COMPACT -> 120
        ScreenClass.MEDIUM -> 140
        ScreenClass.EXPANDED -> 175
    }
    ...
    private fun applyAdaptiveDimensions() {
        ...
        (leftPanel.layoutParams as? LinearLayout.LayoutParams)?.apply {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Locked to landscape (the editing surface needs the width), but SENSOR
        // lets it flip between the two landscape directions instead of forcing
        // one physical orientation — the adaptive part is the panel sizing below.
    ...
    private fun buildUi() {
        ...
        topBar = buildTopBar()
        column.addView(topBar, LinearLayout.LayoutParams(-1, dp(topBarHeightDp())))
        ...
        mainBody.addView(leftPanel, LinearLayout.LayoutParams(dp(leftPanelWidthDp()), -1))
        ...
        mainBody.addView(rightPanel, LinearLayout.LayoutParams(dp(rightPanelWidthDp()), -1))
